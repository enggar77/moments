...
describe('EventDetails', async () => {
	...
	it('renders event details correctly when data is loaded', async () => {
		...
		expect(screen.getAllByText('Summer Concert')).toBeInTheDocument();
		expect(
			screen.getAllByText('A fantastic summer concert experience')
		).toBeInTheDocument();
		expect(screen.getAllByText('Central Park, New York')).toBeInTheDocument();
		expect(screen.getAllByText('£75.00')).toBeInTheDocument();
		expect(screen.getAllByText('350 / 500 left')).toBeInTheDocument();

		// Check for the event information section
		expect(screen.getAllByText('Event Information')).toBeInTheDocument();
		expect(
			screen.getByText(
